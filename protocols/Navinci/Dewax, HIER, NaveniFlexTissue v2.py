...
def puncture_wells(pipette, wells, height_offset=0, top_height_offset=-5, keep_tip=False):
    ...
    for well in wells:
        try:
            iter(well)
            puncture_wells(pipette, well, height_offset, top_height_offset, keep_tip)
        except TypeError:
...
def moveShutter(protocol, pipette, covered_lbwr, keep_tip, use_tip, rate, repeats, top_offset, open):
    ...
    for i in range(repeats):
        for l in [loc1, loc2]:
            pipette.move_to(l, force_direct=True, speed=pipette.default_speed * rate)

    if use_tip and not keep_tip:
        pipette.drop_tip()


def openShutter(protocol, pipette, covered_lbwr, keep_tip=False, use_tip=False, rate=0.3, repeats=3, top_offset=-20):
    moveShutter(protocol, pipette, covered_lbwr, keep_tip, use_tip, rate, repeats, top_offset, open=True)


def closeShutter(protocol, pipette, covered_lbwr, keep_tip=False, use_tip=False, rate=0.3, repeats=3, top_offset=-20):
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if dewax:
        ...
        safe_delay(protocol, minutes=dewax_equilib_delay,
                   msg="equilibrating temp for to {}C for dewaxing".format(dewax_temp))
    ...
    if do_navinci:
        ...
        apply_and_incubate(protocol, pipette, wells.primary_ab, "Primary abs", sample_chambers, stain_volume, 1, 60,
                           new_tip='always')
        apply_and_incubate(protocol, pipette, wash_buffer, "Wash", sample_chambers, wash_volume, 3,
                           wash_incubation_time)
        apply_and_incubate(protocol, pipette, wells.secondary_ab, "Navenibodies", sample_chambers, stain_volume,
                           num_reps, 60)
        apply_and_incubate(protocol, pipette, wash_buffer, "Wash", sample_chambers, wash_volume, num_washes_post_NAB,
                           wash_incubation_time, puncture=False)
        apply_and_incubate(protocol, pipette, wells.ligation, "Ligation", sample_chambers, stain_volume, 1, 30)
        apply_and_incubate(protocol, pipette, wash_buffer, "Wash", sample_chambers, wash_volume, 2,
                           wash_incubation_time, puncture=False)
        ...
        apply_and_incubate(protocol, pipette, wash_buffer, "Wash", sample_chambers, wash_volume, 2,
                           wash_incubation_time, puncture=False)
        temp_mod.quick_temp(incubation_temp)
        apply_and_incubate(protocol, pipette, wells.post_block, "Post-block", sample_chambers, stain_volume, num_reps,
                           15)
