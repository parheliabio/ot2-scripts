...
### END VERAO GLOBAL
metadata = {
    'protocolName': 'ACD RNAscope Multiplex V2 - 4 channels + delayed_start + DAPI',
...
tip_type = 'opentrons_96_filtertiprack_200ul'
...
rna_reagents_plate_1_position = 6
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    wash_wells = list(buffer_wells.values())[0: num_samples]

    DAPI_wells = RNA_reagents_96plate_1.rows()[0]
    # preblock is not currently used
    # preblock_wells = RNA_reagents_96plate_1.rows()[1]
    # rows 3-4 are reserved for future uses
    ...
    anti_dig_wells = RNA_reagents_96plate_3.rows()[7]
    ...
    for i in range(len(sample_chambers)):
        puncture_wells(pipette_300, wash_wells[i], keep_tip=True)
    pipette_300.drop_tip()

    # applying probes and amps

    for z in range(num_amplification_steps):
        ...
        for wash_counter in range(num_washes):
            openShutter(protocol, pipette_300, omnistainer)
            for i in range(len(sample_chambers)):
                washSamples(pipette_300, wash_wells[i], sample_chambers[i], wash_volume, 2,
                            keep_tip=True)
            closeShutter(protocol, pipette_300, omnistainer)
    ...
    for z in range(num_RNAs):
        ...
        if z > 0:  # only done in cycles 2,3 and 4
            ...
            protocol.comment("washing with Parhelia wash buffer")
            for wash_counter in range(3):
                openShutter(protocol, pipette_300, omnistainer)
                for i in range(len(sample_chambers)):
                    washSamples(pipette_300, wash_wells[i], sample_chambers[i], wash_volume, 2, keep_tip=True)
                closeShutter(protocol, pipette_300, omnistainer)
                safe_delay(protocol, minutes=1, msg="incubation in Parhelia wash buffer")
        ...
        for wash_counter in range(4):
            ...
            for i in range(len(sample_chambers)):
                washSamples(pipette_300, wash_wells[i], sample_chambers[i], wash_volume, 2, keep_tip=True)
            closeShutter(protocol, pipette_300, omnistainer)
            safe_delay(protocol, minutes=4, msg="incubation in Parhelia wash buffer")
        ...
        for i in range(len(sample_chambers)):
            ...
            pipette_300.transfer(ab_volume, Substrate_wells[z][i], Tyramide_wells[z][i], new_tip='never',
        ...
        closeShutter(protocol, pipette_300, omnistainer)
        safe_delay(protocol, minutes=30)
    ...
    if num_RNAs == 4:
        protocol.comment("puncturing anti-dig wells of channel " + str(channel))
        for i in range(len(sample_chambers)):
            puncture_wells(pipette_300, anti_dig_wells[i], keep_tip=True)
        protocol.comment("applying the anti-dig antibody")
        openShutter(protocol, pipette_300, omnistainer)
        if double_add:
            for i in range(len(sample_chambers)):
                washSamples(pipette_300, anti_dig_wells[i], sample_chambers[i], ab_volume, 1,
                            keep_tip=keeping_the_tips[z])
            safe_delay(protocol, minutes=1)
        for i in range(len(sample_chambers)):
            washSamples(pipette_300, anti_dig_wells[i], sample_chambers[i], ab_volume, 1,
        ...
        protocol.comment("washing in Parhelia wash buffer")
        for wash_counter in range(3):
            openShutter(protocol, pipette_300, omnistainer)
            for i in range(len(sample_chambers)):
                washSamples(pipette_300, wash_wells[i], sample_chambers[i], wash_volume, 2, keep_tip=True)
    ...
    for i in range(len(sample_chambers)):
        puncture_wells(pipette_300, DAPI_wells[i], keep_tip=True)
        # cycle is needed to puncture only the columns in use

    # Washing with DAPI
    openShutter(protocol, pipette_300, omnistainer)
    pipette_300.transfer(wash_volume, DAPI_wells, sample_chambers, new_tip='once', disposal_vol=0)
    safe_delay(protocol, minutes=10, msg="incubation in DAPI")
    pipette_300.distribute(wash_volume, wash_wells[-1], sample_chambers, new_tip='once', disposal_vol=0)
    closeShutter(protocol, pipette_300, omnistainer)
