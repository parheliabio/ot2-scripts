...
def apply_and_incubate(protocol, pipette, source, reagent_name, target_wells, volume, dispense_repeats, incubation_time,
                       puncture=True, step_repeats=1):
    if puncture:
    ...
    try:
        ...
        if len(source) != len(target_wells):
            raise Exception(
                "the number of source wells ({}) doesn't match the number of target wells ({})".format(len(source),
                                                                                                       len(target_wells)))
    except TypeError:
        one_to_many = True
    ...
    for i in range(step_repeats):
        ...
        if one_to_many:
            pipette.distribute(volume, source, target_wells, new_tip='once', disposal_volume=0, blowout=False)
        else:
            pipette.transfer(volume, source, target_wells, new_tip='always', disposal_volume=0, blowout=False)
...
def distribute_between_samples(pipette, sourceSolutionWell, samples, volume, num_repeats=1, height_offset=0,
                               aspiration_offset=0, dispensing_offset=0, keep_tip=False):
    try:
        iter(samples)
    except TypeError:
        samples = [samples]
    ...
    for i in range(0, num_repeats):
        ...
        for s in samples:
            ...
            pipette.dispense(volume / len(samples), s.bottom(height_offset + dispensing_offset), rate=sample_flow_rate)
...
metadata = {
    'protocolName': 'ACD multiplexed v2 ColdPlate quick test mode',
    'author': 'Parhelia Bio <info@parheliabio.com>',
    'description': 'ACD multiplexed v2 ColdPlate, well-based',
    'apiLevel': '2.14'
}
...
### VERAO VAR NAME='protease (or H2O2) treatment - protease recommended' TYPE=BOOLEAN
...
### VERAO VAR NAME='Amp 1 hybridization time (minutes)' TYPE=NUMBER LBOUND=30 UBOUND=360 DECIMAL=FALSE
amp1_time_minutes = 30

### VERAO VAR NAME='Amp 2 hybridization time (minutes)' TYPE=NUMBER LBOUND=30 UBOUND=360 DECIMAL=FALSE
amp2_time_minutes = 15

### VERAO VAR NAME='Amp 3 hybridization time (minutes)' TYPE=NUMBER LBOUND=30 UBOUND=360 DECIMAL=FALSE
amp3_time_minutes = 30

### VERAO VAR NAME='RNA protocol protease (or H2O2) incubation time (minutes)' TYPE=NUMBER LBOUND=1 UBOUND=900 DECIMAL=FALSE
...
def run(protocol: protocol_api.ProtocolContext):
   ...
   #Setting up ColdPlate temperature module and omni-stainer module
    ...
    protease_or_H2O2wash_wells = RNA_reagents_96plate_1.rows()[0]  # could be used for the H2O2 wash or protease
    ...
    probes_and_amps_incubation_times = [hybridization_time_minutes, amp1_time_minutes, amp2_time_minutes,
                                        amp3_time_minutes]
    ...
    if not (temp_mod is None):
        ...
        protocol.comment(f"bringing samples to hybridization temp: {hyb_temp} C")

    protocol.comment("puncturing protease wells")
    for i in range(len(sample_chambers)):
        puncture_wells(pipette_300, protease_or_H2O2wash_wells[i],
    ...
    if protease_treatment:
        ...
        for i in range(len(sample_chambers)):
            protocol.comment(i)
            washSamples(pipette_300, protease_or_H2O2wash_wells[i], sample_chambers[i], ab_volume, 1, keep_tip=True)
    ...
    if avidin_and_biotin_block:
        ...
        for i in range(len(sample_chambers)):
            puncture_wells(pipette_300, avidinblock_wells[i])

        protocol.comment("applying the Aviden block")
        ...
        for i in range(len(sample_chambers)):
            puncture_wells(pipette_300, biotinblock_wells[i])
        ...
        protocol.delay(minutes=15)
        #protocol.delay(minutes=1)
        #Change which protocol.delay is #'d, and change time variables to 1 out to switch to quick mode
    ...
    for z in range(4):
        ...
        for z in range(num_RNAs):
            ...
            if z > 0:  # only done in cycles 2,3 and 4
                ...
                for i in range(len(sample_chambers)):
                    protocol.comment(i)
                    washSamples(pipette_300, HRPblocker_wells[z - 1][i], sample_chambers[i], ab_volume, 1,
                                keep_tip=True)
                #    pipette_300.drop_tip()
                ...
                protocol.comment("washing in ACD wash buffer")
                # for wash_counter in range(3): #was 3, should it be 3 or 4?
            ...
            # DILUTING AND APPLYING THE Tyramide
            ...
            if has_sheath:
                openShutter(protocol, pipette_300, omnistainer)
            for i in range(len(sample_chambers)):
                dilute_and_apply_fixative(pipette_300, Tyramide_wells[z][i], Substrate_wells[z][i],
                                          Tyramide_wells[z][i],
                                          2 * ab_volume)
            if double_add:
                for i in range(len(sample_chambers)):
