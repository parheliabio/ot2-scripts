...
##### END VERAO GLOBAL #####
metadata = {
    'protocolName': 'ACD multiplexed v2 ColdPlate ParLiquid Integration v1.59',
    'author': 'Parhelia Bio <info@parheliabio.com>',
    'description': 'ACD multiplexed v2 ColdPlate ParLiquid Integration v1.59',
    'apiLevel': '2.13'
}
...
#######################PROTOCOL#################
    ...
    if protease_treatment:
        ...
        for i in range(len(sample_chambers)):
            protocol.comment(i)
            wash_well = liquids.Protease.wells[i]  # Get the specific well for aspirating
    ...
    if avidin_and_biotin_block:
        ...
        puncture_well = liquids.Protease.wells[i]
        puncture_wells(pipette_300, puncture_well, keep_tip=True)
        ...
        for i in range(len(sample_chambers)):
            protocol.comment(i)
            wash_well = liquids.aviblock.wells[i]
            washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1, keep_tip=True)
        ...
        # BIOTIN block
        protocol.comment("puncturing avi-block wells")
        puncture_well = liquids.biotinblock.wells[i]
        puncture_wells(pipette_300, puncture_well, keep_tip=True)

        protocol.comment("applying the biotin")
        for i in range(len(sample_chambers)):
            protocol.comment(i)
            wash_well = liquids.biotinblock.wells[i]
            washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1, keep_tip=True)
        pipette_300.drop_tip()
    ...
    if preblock_incubation:
        ...
        for i in range(len(sample_chambers)):
            puncture_well = liquids.ProbeDiluent.wells[i]
            puncture_wells(pipette_300, puncture_well, keep_tip=True)
        ...
        for i in range(len(sample_chambers)):
            protocol.comment(i)
            wash_well = liquids.ProbeDiluent.wells[i]
            washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1, keep_tip=True)
    ...
    for i in range(len(sample_chambers)):
        puncture_wells(pipette_300, liquids.RNAwash[i], keep_tip=True)
    pipette_300.drop_tip()
    ...
    for z in range(4):
        ...
        for i in range(len(sample_chambers)):
            puncture_well = probes_and_amps[z].wells[i]
            puncture_wells(pipette_300, puncture_well, keep_tip=keeping_the_tips[z])
        ...
        if double_add:
            for i in range(len(sample_chambers)):
                wash_well = probes_and_amps[z].wells[i]
                washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1, keep_tip=keeping_the_tips[z])
    ...
    puncture_wells(pipette_300, liquids.storage)
    ...
    for z in range(num_RNAs):
        ...
        if z > 0:  # only done in cycles 2,3 and 4
            ...
            puncture_well = liquids.HRPblocker[z - 1].wells[i]
            puncture_wells(pipette_300, puncture_well, keep_tip=True)
            ...
            for i in range(len(sample_chambers)):
                protocol.comment(i)
                wash_well = liquids.HRPblocker[z - 1].wells[i]
                washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1, keep_tip=True)
        ...
        for i in range(len(sample_chambers)):
            puncture_well = liquids.HRP[z].wells[i]
            puncture_wells(pipette_300, puncture_well, keep_tip=True)
        ...
        if double_add:
            for i in range(len(sample_chambers)):
                wash_well = liquids.HRP[z].wells[i]
                washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1)
            protocol.delay(minutes=1)
        for i in range(len(sample_chambers)):
            wash_well = liquids.HRP[z].wells[i]
            washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1)
        ...
        for i in range(len(sample_chambers)):
            puncture_well =  liquids.Tyramide[z].wells[i]
            puncture_wells(pipette_300, puncture_well, keep_tip=True)

        protocol.comment("puncturing the substrate buffer wells")
        for i in range(len(sample_chambers)):
            puncture_well = liquids.Substrate[z].wells[i]
            puncture_wells(pipette_300, puncture_well, keep_tip=True)
        ...
        if double_add:
            for i in range(len(sample_chambers)):
                wash_well = liquids.Tyramide[z].wells[i]
                washSamples(pipette_300, wash_well, sample_chambers[i], ab_volume, 1, keep_tip=True)
            protocol.delay(minutes=1)
        for i in range(len(sample_chambers)):
            wash_well = liquids.Tyramide[z].wells[i]
