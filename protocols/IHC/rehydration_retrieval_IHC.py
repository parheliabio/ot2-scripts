...
labwarePositions.heatmodule = omnistainer_position
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if rehydration:
        ...
        puncture_wells(pipette_300, buffers.eth_70perc, height_offset=30, keep_tip=True)
    ...
    if type_of_protocol in ['IHC only', 'IHC with Hematoxylin']:
        ...
        for i in range(num_samples):
            puncture_wells(pipette_300, enzymeblock_wells[i], height_offset=12, keep_tip=True)
        pipette_300.drop_tip()
        ...
        for i in range(num_samples):
            puncture_wells(pipette_300, preblock_wells[i], height_offset=12, keep_tip=True)
        pipette_300.drop_tip()
        ...
        for i in range(num_samples):
            protocol.comment(i)
            puncture_wells(pipette_300, antibody_wells[i], height_offset=12)
        ...
        for i in range(num_samples):
            puncture_wells(pipette_300, hrpsecondaryab_wells[i], height_offset=12, keep_tip=True)
        pipette_300.drop_tip()
        ...
        for i in range(num_samples):
            puncture_wells(pipette_300, DAB_wells[i], height_offset=12, keep_tip=True)
            puncture_wells(pipette_300, substrate_wells[i], height_offset=12, keep_tip=True)
        pipette_300.drop_tip()
    ...
    if type_of_protocol in ['IHC with Hematoxylin', 'Hematoxylin only']:
        ...
        if hematoxylin_source == 'from pcr strip':
            ...
            for i in range(num_samples):
                puncture_wells(pipette_300, hematoxylin_wells[i], height_offset=12, keep_tip=True)
            pipette_300.drop_tip()
