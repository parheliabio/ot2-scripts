...
### VERAO VAR NAME='1st Wash (before preblock) Step Repeats (with delay in between)' TYPE=NUMBER LBOUND=1 UBOUND=25
wash_step_reps_1 = 3

### VERAO VAR NAME='2nd Wash (after primary ab) Step Repeats  (with delay in between)' TYPE=NUMBER LBOUND=1 UBOUND=25
wash_step_reps_2 = 3

### VERAO VAR NAME='3rd Wash (after secondary ab) Step Repeats  (with delay in between)' TYPE=NUMBER LBOUND=1 UBOUND=25
wash_step_reps_3 = 3

### VERAO VAR NAME='4th Wash (after tertiary) Step Repeats  (with delay in between)' TYPE=NUMBER LBOUND=1 UBOUND=25
wash_step_reps_4 = 3

### VERAO VAR NAME='5th Wash (after stain) Step Repeats  (with delay in between)' TYPE=NUMBER LBOUND=1 UBOUND=25
wash_step_reps_5 = 3
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    if pre_staining:
        apply_and_incubate(protocol, pipette, wash_buffer_1,            "1x PBS Wash",                  sample_chambers,    wash_volume,    wash_step_reps_1,   wash_incubation,            dispense_repeats = wash_dispense_reps)
        apply_and_incubate(protocol, pipette, antibody_diluent_wells,   "1x Ab Diluent (Preblock)",     sample_chambers,    ab_volume,      single_rep,         ab_diluent_incubation_min,  dispense_repeats = num_reps, new_tip=tip_change_policy)

    if primary_staining:
        apply_and_incubate(protocol, pipette, primary_ab_wells,         "1x Primary Antibody mix",      sample_chambers,    ab_volume,      single_rep,         primary_ab_incubation,      dispense_repeats = num_reps, new_tip=tip_change_policy)
        apply_and_incubate(protocol, pipette, wash_buffer_2,            "1x PBS Wash",                  sample_chambers,    wash_volume,    wash_step_reps_2,   wash_incubation,            dispense_repeats = wash_dispense_reps )

    if secondary_staining:
        apply_and_incubate(protocol, pipette, secondary_ab_wells,       "1x Secondary Antibody Mix",    sample_chambers,    ab_volume,      single_rep,         secondary_ab_incubation,    dispense_repeats = num_reps, new_tip=tip_change_policy)
        apply_and_incubate(protocol, pipette, wash_buffer_3,            "1x PBS Wash",                  sample_chambers,    wash_volume,    wash_step_reps_3,   wash_incubation,            dispense_repeats = wash_dispense_reps )

    if tertiary_staining:
        apply_and_incubate(protocol, pipette, tertiary_ab_wells,        "1x Tertiary Antibody Mix",     sample_chambers,    ab_volume,      single_rep,         tertiary_ab_incubation,     dispense_repeats = num_reps, new_tip=tip_change_policy)
        apply_and_incubate(protocol, pipette, wash_buffer_4,            "1x PBS Wash",                  sample_chambers,    wash_volume,    wash_step_reps_4,   wash_incubation,            dispense_repeats = wash_dispense_reps )

    if counter_staining:
        apply_and_incubate(protocol, pipette, counterstain_wells,       "1x Countertain",               sample_chambers,    ab_volume,      single_rep,         counterstain_incubation,    dispense_repeats = num_reps, new_tip=tip_change_policy)
        apply_and_incubate(protocol, pipette, wash_buffer_5,            "1x PBS Wash",                  sample_chambers,    wash_volume,    wash_step_reps_5,   wash_incubation,            dispense_repeats = wash_dispense_reps )
