...
def washSamples(pipette, sourceSolutionWell, samples, volume, num_repeats=1, dispense_bottom_gap=0):
...
#################PROTOCOL####################
    protocol.home()
    if retreaval:
        washSamples(pipette_300, buffers.retreaval, sample_chambers, wash_volume, 2,0)
        temp_mod.set_temperature(95)
        print("retreaval")
        protocol.delay(minutes=15)
        washSamples(pipette_300, buffers.retreaval, sample_chambers, wash_volume, 1,0)
        protocol.delay(minutes=15)
        washSamples(pipette_300, buffers.retreaval, sample_chambers, wash_volume, 1,0)
        protocol.delay(minutes=15)
...
#WASHING SAMPLES WITH TBS
    ...
    washSamples(pipette_300, buffers.TBS_wash, sample_chambers, wash_volume, 2,0)
...
#Preblocking
    ...
    for i in range(len(wellslist)):
        print(i)
        washSamples(pipette_300, preblock_wells[i], sample_chambers[i], wash_volume, 1, 0)
...
#APPLYING ANTIBODY COCKTAILS TO SAMPLES
    ...
    for i in range(len(wellslist)):
        print(i)
        washSamples(pipette_300, antibody_wells[i], sample_chambers[i], wash_volume, 1, 0)
...
#three individual repeats below is because they need particular incubation time between them
    ...
    for i in range (5):
        washSamples(pipette_300, buffers.TBS_wash, sample_chambers, wash_volume, 1, 0)
...
#APPLYING enzyme blocking
    ...
    for i in range(len(wellslist)):
        washSamples(pipette_300, enzymeblock_wells[i], sample_chambers[i], wash_volume,1, 0)
#INCUBATE 10 MIN
    print("hrp blocking incubation: 10min" )
    protocol.delay(minutes=10)
    washSamples(pipette_300, buffers.TBS_wash, sample_chambers, wash_volume, 3, 0)

#APPLYING HRP SECONDARY ANTIBODY COCKTAILS TO SAMPLES
    print("applying hrpsecondaryab")
    for i in range(len(wellslist)):
        washSamples(pipette_300, hrpsecondaryab_wells[i], sample_chambers[i], wash_volume,1,0)
...
#DILUTING AND APPLYING THE DAB
    ...
    washSamples(pipette_300, buffers.water, sample_chambers, wash_volume, 5,0)


#STORAGE, washing samples every hour
    for i in range (48):
        washSamples(pipette_300, buffers.storage,sample_chambers, 100,1,0)
