...
def washSamples(pipette, sourceSolutionWell, samples, volume, num_repeats=1, height_offset=0, aspiration_offset=0,
...
type_of_protocol = 'IHC with Hematoxylin'
...
### VERAO VAR NAME='labwarePositions.buffers_plate_2' TYPE=NUMBER LBOUND=1 UBOUND=12 DECIMAL=FALSE
buffers_plate_2_position = 2
...
labwarePositions.buffers_plate_2 = buffers_plate_2_position
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    trough12_2 = protocol.load_labware('parhelia_12trough', labwarePositions.buffers_plate_2,
                                       '12-trough buffers reservoir_2')
    ...
    buffer_wells_2 = trough12_2.wells_by_name()
    ...
    buffers.oil1 = buffer_wells_2['A1']
    buffers.oil2 = buffer_wells_2['A2']
    buffers.oil3 = buffer_wells_2['A3']
    buffers.oil4 = buffer_wells_2['A4']
    buffers.oil5 = buffer_wells_2['A5']
    buffers.oil6 = buffer_wells_2['A6']
    buffers.oil7 = buffer_wells_2['A7']
    ...
    if baking:
        ...
        protocol.delay(minutes=80)
        temp_mod.set_temperature(25)
        protocol.delay(minutes=20)

    if rehydration:

        puncture_wells(pipette_300, buffers.oil1, height_offset=-5, keep_tip=True)
        puncture_wells(pipette_300, buffers.oil2, height_offset=-5, keep_tip=True)
        puncture_wells(pipette_300, buffers.oil3, height_offset=-5, keep_tip=True)
        puncture_wells(pipette_300, buffers.oil4, height_offset=-5, keep_tip=True)
        puncture_wells(pipette_300, buffers.oil5, height_offset=-5, keep_tip=True)
        puncture_wells(pipette_300, buffers.oil6, height_offset=-5, keep_tip=True)
        puncture_wells(pipette_300, buffers.oil7, height_offset=-5)
        ...
        puncture_wells(pipette_300, buffers.storage, height_offset=-5, keep_tip=True)
        ...
        for i in range(3):
            washSamples(pipette_300, buffers.oil1, sample_chambers[0], wash_volume, 1)
            washSamples(pipette_300, buffers.oil2, sample_chambers[1], wash_volume, 1)
            washSamples(pipette_300, buffers.oil3, sample_chambers[2], wash_volume, 1)
            washSamples(pipette_300, buffers.oil4, sample_chambers[3], wash_volume, 1)
            washSamples(pipette_300, buffers.oil5, sample_chambers[4], wash_volume, 1)
            washSamples(pipette_300, buffers.oil6, sample_chambers[5], wash_volume, 1)
            washSamples(pipette_300, buffers.oil7, sample_chambers[6], wash_volume, 1)
        ...
        protocol.pause(msg="The protocol is paused for washing the OMNIstainer from oils")
    ...
    if retrieval:
        ...
        temp_mod.set_temperature(76)
        for i in range(1, 11):
            protocol.comment(i)
            temp_mod.set_temperature(76 + 2 * i)
            protocol.delay(40)
