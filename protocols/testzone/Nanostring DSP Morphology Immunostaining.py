...
def run(protocol: protocol_api.ProtocolContext):
    ...
    wash_buffer = codex_trough12.wells_by_name()['A11']

    buffer_W_wells = reagents_96plate.rows()[0]
    primary_ab_wells = reagents_96plate.rows()[1]
    secondary_ab_wells = reagents_96plate.rows()[2]
    conj_syto_wells = reagents_96plate.rows()[3]
    ...
    # 30 min buffer W
    reagent_name = "buffer W"
    protocol.comment("applying " + reagent_name)
    washSamples(pipette, buffer_W_wells, sample_chambers, ab_volume, 1)
    protocol.delay(minutes=buffer_w_incubation, msg = reagent_name + " incubation")
    
    # 1h primary
    reagent_name = "primary Ab"
    protocol.comment("applying " + reagent_name)
    washSamples(pipette, primary_ab_wells, sample_chambers, ab_volume, 1)
    protocol.delay(minutes=primary_ab_incubation, msg = reagent_name + " incubation")
    
    # 5 min wash (2x)
    for i in range (2)
        reagent_name = "1x SSC Wash"
        protocol.comment("applying " + reagent_name)
        washSamples(pipette, wash_buffer, sample_chambers, wash_volume, 2)
        protocol.delay(minutes=wash_incubation, msg = reagent_name + " incubation")
    
    # 30 min secondary
    reagent_name = "secondary Ab"
    protocol.comment("applying " + reagent_name)
    washSamples(pipette, secondary_ab_wells, sample_chambers, ab_volume, 1)
    protocol.delay(minutes=secondary_ab_incubation, msg = reagent_name + " incubation")
    ...
    # 30 min conjugated + syto
    reagent_name = "conjugated + syto"
    protocol.comment("applying " + reagent_name)
    washSamples(pipette, conj_syto_wells, sample_chambers, ab_volume, 1)
    protocol.delay(minutes=conj_syto_incubation, msg = reagent_name + " incubation")
    conj_syto_incubation
