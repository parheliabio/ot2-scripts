...
## Last updated by nsamusik January 22, 2024
...
### END VERAO GLOBAL
metadata = {
    'protocolName': 'Bake-Dewax-AR-IMC with quick_temp, 1 min IMC incubations v1.0.3',
    'author': 'Parhelia Bio <info@parheliabio.com>',
    'description': 'Automation of IMC staining protocols from Ali Group team, manual pauses enable fridge incubations, also 96-DWP.',
    'apiLevel': '2.14'
}
...
### VERAO VAR NAME='Device type' TYPE=CHOICE OPTIONS=['omni_stainer_s12_slides_with_thermosheath_on_coldplate', 'omni_stainer_s12_slides_with_thermosheath', 'omni_stainer_s12_slides']
omnistainer_type = 'omni_stainer_s12_slides_with_thermosheath_on_coldplate'

### VERAO VAR NAME='Reagent well plate type' TYPE=CHOICE OPTIONS=['parhelia_skirted_96', 'parhelia_skirted_96_with_strips', 'nest_96_wellplate_2ml_deep']
type_of_96well_plate = 'nest_96_wellplate_2ml_deep'
...
### VERAO VAR NAME='Double add of antibody/reagents (uses 2x ab_volume, but improves uniformity)' TYPE=BOOLEAN
double_add = False
...
### VERAO VAR NAME='Doing Dewax on Omni-Stainer?' TYPE=BOOLEAN
dewax = True

### VERAO VAR NAME='Dewaxing temperature (C)' TYPE=NUMBER LBOUND=10 UBOUND=90  DECIMAL=TRUE INCREMENT=0.1
dewax_temp = 72

### VERAO VAR NAME='Dewax fill slowdown factor' TYPE=NUMBER LBOUND=1 UBOUND=20 DECIMAL=FALSE
dispense_slowdown_factor = 1

### VERAO VAR NAME='Alcohol washing temp, pre-HIER (C)' TYPE=NUMBER LBOUND=10 UBOUND=80 DECIMAL=TRUE INCREMENT=0.1
alc_wash_temp = 32.0

### VERAO VAR NAME='Doing Heat-Induced Antigen/Epitope Retrieval on Omni-Stainer?' TYPE=BOOLEAN
HIER = True

### VERAO VAR NAME='Antigen Retrieval Buffer' TYPE=CHOICE OPTIONS=['ER1','ER2']
ER_buffer = 'ER2'

### VERAO VAR NAME='Antigen Retrieval Temperature (C)' TYPE=NUMBER LBOUND=1 UBOUND=99  DECIMAL=TRUE INCREMENT=0.1
retrieval_temp = 95.0

### VERAO VAR NAME='Retrieval Time (min)' TYPE=NUMBER LBOUND=1 UBOUND=480  DECIMAL=TRUE INCREMENT=0.1
retrieval_time = 30.0

### VERAO VAR NAME='Room temp (C)' TYPE=NUMBER LBOUND=20 UBOUND=99 DECIMAL=FALSEDECIMAL=TRUE INCREMENT=0.1
room_temp = 22.0

### VERAO VAR NAME='Sample flow rate' TYPE=NUMBER LBOUND=0.05 UBOUND=1.0 DECIMAL=TRUE INCREMENT=0.05
sample_flow_rate = 0.2

### VERAO VAR NAME='Optional Pause after Antigen Retrieval to remove waste fluid?' TYPE=BOOLEAN
post_HIER_pause = True
...
### VERAO VAR NAME='Apply Antibody Cocktail?' TYPE=BOOLEAN
antibody_cocktail = True

### VERAO VAR NAME='Perform Nuclear Staining?' TYPE=BOOLEAN
counter_staining = True

### VERAO VAR NAME='Blocking Buffer Incubation time (min)' TYPE=NUMBER LBOUND=1 UBOUND=4200 DECIMAL=FALSE
blocking_buffer_incubation_min = 1

### VERAO VAR NAME='Primary Antibody Incubation temp (C)' TYPE=NUMBER LBOUND=1 UBOUND=99 DECIMAL=TRUE INCREMENT=0.1
primary_ab_temp = 17.2

### VERAO VAR NAME='MANUAL PAUSE during the primary antibody incubation' TYPE=BOOLEAN
primary_protocol_pause = False

### VERAO VAR NAME='Primary Antibody Incubation time (min) (if NOT MANUAL PAUSE)' TYPE=NUMBER LBOUND=1 UBOUND=4200 DECIMAL=FALSE
primary_ab_incubation = 1

### VERAO VAR NAME='Secondary Antibody Incubation temp (C)' TYPE=NUMBER LBOUND=1 UBOUND=99 DECIMAL=TRUE INCREMENT=0.1
secondary_ab_temp = 22.0

### VERAO VAR NAME='Secondary Antibody Incubation time (min)' TYPE=NUMBER LBOUND=1 UBOUND=4200 DECIMAL=FALSE
secondary_ab_incubation = 1

### VERAO VAR NAME='Antibody Cocktail Incubation temp (C)' TYPE=NUMBER LBOUND=1 UBOUND=99 DECIMAL=TRUE INCREMENT=0.1
ab_cocktail_temp = 4.5

### VERAO VAR NAME='MANUAL PAUSE during the Antibody Cocktail incubation' TYPE=BOOLEAN
cocktail_protocol_pause = False

### VERAO VAR NAME='Antibody Cocktail Incubation time (min)(if NOT MANUAL PAUSE)' TYPE=NUMBER LBOUND=1 UBOUND=4200 DECIMAL=FALSE
ab_cocktail_incubation = 1

### VERAO VAR NAME='Nuclear Counterstain incubation time (min)' TYPE=NUMBER LBOUND=1 UBOUND=4200 DECIMAL=FALSE
counterstain_incubation = 1
...
### VERAO VAR NAME='temp_mod number' TYPE=NUMBER LBOUND=0 UBOUND=10 DECIMAL=FALSE
temp_mod_number = 3
...
### VERAO VAR NAME='DECK POSITION: Antigen Retrieval & IMC Buffers plate' TYPE=NUMBER LBOUND=1 UBOUND=12 DECIMAL=FALSE
HIER_buffers_plate_position = 7

### VERAO VAR NAME='DECK POSITION: Staining Module' TYPE=NUMBER LBOUND=1 UBOUND=12 DECIMAL=FALSE
omnistainer_position = 1

### VERAO VAR NAME='DECK POSITION: IMC reagents 96-well plate' TYPE=NUMBER LBOUND=1 UBOUND=12 DECIMAL=FALSE
IMC_reagents_96plate_position = 8
...
### VERAO VAR NAME='Tiprack 1 starting position' TYPE=NUMBER LBOUND=1 UBOUND=95 DECIMAL=FALSE
tiprack_300_starting_pos = 1

### VERAO VAR NAME='DECK POSITION:Tiprack 2' TYPE=NUMBER LBOUND=1 UBOUND=12 DECIMAL=FALSE
...
### VERAO VAR NAME='Protocol start delay time (minutes)' TYPE=NUMBER LBOUND=1 UBOUND=8000 DECIMAL=FALSE
...
labwarePositions.HIER_buffers_plate = HIER_buffers_plate_position
labwarePositions.IMC_reagents_96plate = IMC_reagents_96plate_position
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # HIER Buffers plate
    HIER_buffers_wells = protocol.load_labware('parhelia_12trough', labwarePositions.HIER_buffers_plate,
                                               '12-trough reservoir')

    # IMC Buffers plate
    # IMC_buffers_wells = protocol.load_labware('parhelia_12trough', labwarePositions.buffers_plate,
    #                                       '12-trough reservoir')
    # Reagent plate
    IMC_reagents_96plate = protocol.load_labware(type_of_96well_plate, labwarePositions.IMC_reagents_96plate,
                                                 '96-well-plate')
    ...
    # Antigen Retrieval Buffers Plate Reagents
    HIER_wells = HIER_buffers_wells.wells_by_name()
    ...
    buffers.TBS_T = HIER_wells['A8']
    buffers.TBS_T_2 = HIER_wells['A9']
    buffers.TBS = HIER_wells['A10']

    # IMC Buffers Plate Reagents
    # buffer_wells_2 = IMC_buffers_wells.wells_by_name()
    #
    # buffers.TBS_T = buffer_wells_2['A1']
    # buffers.TBS = buffer_wells_2['A2']
    # buffers.Blocking = buffer_wells_2['A3']
    # buffers.methanol = buffer_wells_2['A11']
    # buffers.fixPFA = buffer_wells_2['A12']

    # IMC reagents plate reagents

    if 'parhelia_skirted_96' in type_of_96well_plate:
        blocking_buffer_well = IMC_reagents_96plate.rows()[0][:num_samples]
        primary_ab_well = IMC_reagents_96plate.rows()[1][:num_samples]
        secondary_ab_well = IMC_reagents_96plate.rows()[2][:num_samples]
        ab_cocktail_well = IMC_reagents_96plate.rows()[3][:num_samples]
        counterstain_wells = IMC_reagents_96plate.rows.rows()[4][:num_samples]
        # fluor_probe_wells = IMC_reagents_96plate.rows()[5][:num_samples]
        # fluor_probe_2_wells = IMC_reagents_96plate.rows()[6][:num_samples]

    if '2ml_deep' in type_of_96well_plate:
        IMC_wells = IMC_reagents_96plate.wells_by_name()

        blocking_buffer_well = IMC_wells['B2']
        primary_ab_well = IMC_wells['C2']
        secondary_ab_well = IMC_wells['D2']
        ab_cocktail_well = IMC_wells['E2']
        counterstain_wells = IMC_wells['F2']
    ...
    if HIER:
        ...
        for i in range(reps):
            safe_delay(protocol, minutes=5,
                       msg="heating up to " + str(retrieval_temp) + ", topping off ER buffer as we go" + str(
                           i + 1) + "/" + str(reps))
            openShutter(protocol, pipette, omnistainer)
            # distribute_between_samples(pipette, er_buff_well, sample_chambers, wash_volume / 2, 1, keep_tip=True)
            distribute_between_samples(pipette, er_buff_well, sample_chambers, wash_volume, 1, keep_tip=True)
            closeShutter(protocol, pipette, omnistainer)
            protocol.home()

        safe_delay(protocol, minutes=retrieval_time, msg="Antigen Retrieval in progress")
        ...
        cooldown_delay_min = 1
        topoff_every_min = 5

        for i in range(int(cooldown_delay_min / topoff_every_min)):
            safe_delay(protocol, minutes=topoff_every_min,
                       msg="adjusting temp to " + str(target_temp) + ", topping off ER buffer to prevent evap")
            openShutter(protocol, pipette, omnistainer)
            # distribute_between_samples(pipette, er_buff_well, sample_chambers, wash_volume / 2, 1, keep_tip=True)
            distribute_between_samples(pipette, er_buff_well, sample_chambers, wash_volume, 1, keep_tip=True)
            closeShutter(protocol, pipette, omnistainer)
        ...
        for i in range(2):
            washSamples(pipette, buffers.PBS, sample_chambers, wash_volume, 2, keep_tip=True)
            safe_delay(protocol, minutes=2, msg="Incubating in PBS #" + str(i))
