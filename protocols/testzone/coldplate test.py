...
metadata = {
    'protocolName': 'temp test coldplate V4 , 7.2 hr, overshoot 5T at 0.4t templag 60 min',
...
### VERAO VAR NAME='Hot temperature' TYPE=NUMBER LBOUND=1 UBOUND=99 DECIMAL=FALSE
hot_temp = 99

### VERAO VAR NAME='cold temperature' TYPE=NUMBER LBOUND=1 UBOUND=99 DECIMAL=FALSE
cold_temp = 1
...
### VERAO VAR NAME='Overshot temperature set_temp will be this much above or below actual goal temp for fraction of templag set by overshot_time_factor)' TYPE=NUMBER LBOUND=0 UBOUND=95 DECIMAL=FALSE
overshot_temp = 5

### VERAO VAR NAME='overshot_time_factor, the fraction of templag to overshoot the temp' TYPE=NUMBER LBOUND=0 UBOUND=1 DECIMAL=TRUE
overshot_time_factor = 0.15
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    protocol.comment(f"Temp Test set_temp. time = 7 * (std delay = {templag_time_minutes})")
    protocol.home()
    ...
    if not (temp_mod is None):
        ...
        protocol.comment(f"Temperature going to {hyb_temp} C, {templag_time_minutes * (1-overshot_time_factor)} min")
        temp_mod.set_temp(hyb_temp)
        protocol.delay(minutes=templag_time_minutes * (1-overshot_time_factor))
        #
        temp_mod.set_temp(room_temp - overshot_temp)
        protocol.delay(minutes=templag_time_minutes * overshot_time_factor)
        protocol.comment(f"Temperature going to {room_temp} C, {templag_time_minutes} min")
        temp_mod.set_temp(room_temp)
        protocol.delay(minutes=templag_time_minutes * (1-overshot_time_factor))
        #
        temp_mod.set_temp(hyb_temp + overshot_temp)
        protocol.delay(minutes=templag_time_minutes * overshot_time_factor)
        protocol.comment(f"Temperature going to {hyb_temp} C, {templag_time_minutes} min")
        temp_mod.set_temp(hyb_temp)
        protocol.delay(minutes=templag_time_minutes * (1 - overshot_time_factor))
        #
        temp_mod.set_temp(hot_temp)
        protocol.comment(f"Temperature going to {hot_temp} C, {templag_time_minutes} min")
        protocol.delay(minutes=templag_time_minutes)
        #
        temp_mod.set_temp(room_temp)
        protocol.comment(f"Temperature going to {room_temp} C, {templag_time_minutes} min")
        protocol.delay(minutes=templag_time_minutes)
        #
        temp_mod.set_temp(cold_temp)
        protocol.comment(f"Temperature going to {cold_temp} C, {templag_time_minutes} min")
        protocol.delay(minutes=templag_time_minutes)
        #
        temp_mod.set_temp(storage_temp)
        protocol.comment(f"Temperature going to {storage_temp} C, {templag_time_minutes} min")
        protocol.delay(minutes=templag_time_minutes)
        #
        temp_mod.temp_off()
