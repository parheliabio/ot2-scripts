...
### END VERAO GLOBAL
metadata = {
    'protocolName': 'Parhelia Dewaxing, Rehydration & H&E v6',
    'author': 'Parhelia Bio <info@parheliabio.com>',
    'description': 'Parhelia Universal Dewaxing, Rehydration & H&E protocol for slides (inc. 10x Visium), coverslips and Akoya PCF flowcells',
    'apiLevel': '2.14'
}

####################MODIFIABLE RUN PARAMETERS#########################

### VERAO VAR NAME='Device type' TYPE=CHOICE OPTIONS=['omni_stainer_s12_pcf_flowcells', 'omni_stainer_s12_slides', 'omni_stainer_c12_cslps']
omnistainer_type = 'omni_stainer_s12_pcf_flowcells'

### VERAO VAR NAME='Number of Samples' TYPE=NUMBER LBOUND=1 UBOUND=12 DECIMAL=FALSE
num_samples = 1

### VERAO VAR NAME='Skip dewaxing' TYPE=BOOLEAN
skip_dewax = True

### VERAO VAR NAME='Skip rehydration' TYPE=BOOLEAN
skip_rehydration = True

### VERAO VAR NAME='Mount in Dewaxing solution' TYPE=BOOLEAN
mount_in_dewax = False
...
### VERAO VAR NAME='Sample flow rate: set to 0.2 for slides, 0.07 for cslps and PCF flowcells' TYPE=NUMBER LBOUND=0.01 UBOUND=1 DECIMAL=TRUE INCREMENT=0.01
sample_flow_rate = 0.1

### VERAO VAR NAME='Hematoxylin incubation time' TYPE=NUMBER LBOUND=1 UBOUND=5 DECIMAL=FALSE
hematox_delay = 3

### VERAO VAR NAME='post-Hematoxylin differentiation in water time' TYPE=NUMBER LBOUND=1 UBOUND=5 DECIMAL=FALSE
diff_time = 1
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    pipette = protocol.load_instrument('p300_single_gen2' if pipette_300_GEN == 'GEN2' else 'p300_single',
    ...
    reagent_sequence = ['Dewax', '100% EtOH', '95% EtOH', '70% EtOH', 'Water', 'Hematoxylin', 'Water',
                        'Blueing reagent', 'Water', 'Eosin', '95% EtOH', '100% EtOH', 'Dewax']
    delay_mins = [10, 3, 3, 3, 0, hematox_delay, diff_time, 1, 0, eosin_delay, 0, 0, 0]
    reps = [2, 2, 2, 2, 2, 1, 5, 1, 5, 1, 2, 2, 1]
    speeds = [1, 0.7, 0.7, 0.7, 1, 1, 1, 1, 1, 0.7, 0.7, 0.7, 0.7]
    ...
    for i in range(len(reagent_sequence)):
        if skip_dewax and i == 0:
            continue

        if skip_rehydration and i < 4:
            continue

        if not mount_in_dewax and i == 12:
            continue
        ...
        for j in range(reps[i]):
            protocol.comment(
                "Applying reagent " + reagent + " from well " + str(well) + " to the sample, repeat " + str(
                    j + 1) + " out of " + str(reps[i] + 1))
            ...
            protocol.comment("Dispensing took " + str(elapsed) + " sec, incubating for remaining: " + str(
                delay_seconds) + " out of total " + str(delay_mins[i] * 60) + " sec")

            if j == reps[i] - 1 and pipette.has_tip:
                pipette.drop_tip()
